...
All images, the Smart Corridors services and the VPP (ex-SmartFace) platform, come from a single registry. Before the first run:
...
`vpp/` holds the VPP (formerly SmartFace) deployment: `vpp/.env` for platform settings, `vpp/.env.sfstation` for SmartFace Station and `vpp/run.sh`, which starts the dependencies, migrates the database and starts the services. Images are pulled from `registry.dot.innovatrics.com/border-control/vpp/`.

The platform is pinned to `SF_VERSION=v5_4.41.0.9767-dev` on purpose. It is the last release whose containers run as root. From v5_4.41.1 the images run as uid 10001, and the licensing library then computes a hardware ID that differs from the one printed by `license-manager`, so licenses issued for the documented HWID are rejected with `License has different HWID than this machine`. Do not bump `SF_VERSION` until VPP ships a HWID tool that matches its services.
